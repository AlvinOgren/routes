...
vm.runInContext('state.nearby=true;',context);controls.sort.value='new';controls['race-filter'].checked=true;controls['segment-filter'].checked=true;
assert.equal(context.filtered().map(r=>r.id).join(','),'near,far');
assert.equal(context.filtered().length,2); // Nearby sorting must not impose a distance cutoff.
assert.ok(Math.abs(context.startDistance({preview:[[59,15]]})-111.195)<.01);
console.log('Nearby sorting without cutoff and combined MTB/race/segment filters passed.');
