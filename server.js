...
export function createApp({dataDir=path.join(ROOT,'data'),password=process.env.ADMIN_PASSWORD,publicURL=process.env.PUBLIC_URL||'',suggest=fetchSuggestions}={}){
 ...
 // Existing Python route table and payload format are used without a migration.
 ...
 app.post('/api/routes/:id/suggest-surfaces',admin,async(req,res)=>{const r=load(req.params.id),b=body(req);if(b.revision!==r.revision)throw fail('Öppna rutten på nytt.',409);if(osmBusy||Date.now()-osmLast<60000)throw fail('Vänta en minut mellan karthämtningarna.',429);osmBusy=true;osmLast=Date.now();try{const result=await suggest(r.points);let spans=result.spans;for(const s of r.surfaces)if(s.source==='manual')spans=paint(spans,s.start,s.end,s.kind,r.distance);if(load(r.id).revision!==r.revision)throw fail('Rutten ändrades under hämtningen. Inga förslag sparades.',409);r.surfaces=spans;r.surface_diagnostics=result.diagnostics;save(r);res.json(r);}finally{osmBusy=false;}});
